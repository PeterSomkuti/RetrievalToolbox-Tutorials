...
The role of the wavelength grid is quite important: all optical property and radiative transfer calculations will be performed on that grid. Overall computing performance tends to scale linearly with the number of points in the spectral grid - so doubling the number of points of the spectral window roughly leads to a doubling of the overall forward model execution time.

## Wavelengths and Wavenumbers

RetrievalToolbox supports both wavelength and wavenumbers as choices for the spectral dimension natively. Additional details are also found in the [RetrievalToolbox documentation](https://us-ghg-center.github.io/RetrievalToolbox.jl/dev/design/design/#Wavelengths-and-wavenumbers).
...
In many scripting applications, we would use dictionaries in a manner like above where we link strings or integers to other, generally simple objects. In Julia, many more types can be used as dictionary keys, not just numbers, strings or tuples. In Python, only immutable objects may by hashable, whereas in Julia, even arrays can be hashed, and their hash will obviously change depending on the contents of the arrays. To create dictionaries in Julia, we can even use our own types! We will use a quick example by creating two new custom types, and use variables of those types as keys in dictionaries.
...
Let us try something very similar: we will create a new object, that is seemingly the same object, and try to access the dictonary in exactly the same way. However, we will see that Julia raises an error!
...
We fill these in with some plausible values. Note that the satellite velocity and satellite position fields are currently unused, so we can safely fill them with zeros. Here we also want to point users the RetrievalToolbox documentation about the problematic ambiguities regarding angular units: [see here](https://us-ghg-center.github.io/RetrievalToolbox.jl/dev/pitfalls/pitfalls/#Angles-and-trigonometric-functions-with-Unitful.jl). All angular units, such as zenith and azimuth angles, should be entered as degrees without any `Unitful` units attached to them as to avoid any implicit unwanted unit conversions between degrees and radians.
...
RE.ingest!(atm, :met_pressure, [5., 65., 200., 400., 650., 950.]u"hPa")
RE.ingest!(atm, :specific_humidity, [0.0001, 0.0002, 0.00035, 0.00035, 0.00075, 0.0020]u"kg/kg")
RE.ingest!(atm, :temperature, [253., 233., 238., 253., 278., 293.]u"K")
...
```{julia}
p1 = plot(atm.altitude * atm.altitude_unit, collect(1:atm.N_met_level),
    lw=2.0, marker=:circle, label="Altitude profile", legend=:bottomright)
ylabel!("Met profile level #");
yflip!()
p2 = plot(atm.gravity * atm.gravity_unit, collect(1:atm.N_met_level),
