...
In the last section, we filled our atmosphere object with values for the retrieval pressure grid: `atm.pressure_levels`. Inspecting the type documentation ([link](https://us-ghg-center.github.io/RetrievalToolbox.jl/dev/types/atmosphere_types/)), we can see that there is a field `atm.pressure_layers`, which still contains only zeros:
...
Since this is a common operation, RetrievalToolbox has a convenience function `levels_to_layers` which calculates the mid-layer values for a given level-based profile.
...
At this point, the atmosphere object `atm` has reasonable values for retrieval pressure grid.

::: {.callout-warning}
**Important information on the meteorological profiles follows!**
:::

While we explicitly differentiate between levels and layers for the **retrieval grid**, there is **no such distinction for the metorological profiles**. Meteorological profiles are often derived from weather or climate models which often use term "levels" in a different manner. In the NWP community, a "model level" often refers to a slab with finite thickness, whereas a retrieval grid level here has no vertical extent.

For data from models such as NASA's GEOS or ECWMF's IFS, quantities like specific humidity or temperature are reported on these model levels that are related to some *mid-level pressure*, despite the fact they generally represent the mean of the model slab with finite thickness. We recommend to simply use those (model) mid-level pressures to become the vertical coordinate of your meteorological profiles. In our `EarthAtmosphere` object, that vertical coordinate is accessed through `.met_pressure`. Note that `.met_pressure` does not have a *level* or *layer* aspect here, to avoid confusion. Internally, RetrievalToolbox performs interpolation to obtain the meteorological quantities at any pressure, thus assuming that those quantities are smoothly varying between the known points of these mid-level pressures.

Remember, that we initialized the atmosphere object with two more levels than the retrieval grid:

```{julia}
mgrid = [5., 65., 200., 400., 650., 950.]u"hPa"

atm.met_pressure[:] = ustrip(mgrid .|> atm.met_pressure_unit);
```
...
With this knowledge, we can fill in the specific humidity profile with a unit-carrying vector, and we also make sure to not forget to calculate the mid-layer values. Note again that the specific humidity profile is accessed through `.specific_humidity`, without a notion of level or layer.

```{julia}
q = [0.0001, 0.0002, 0.00035, 0.00035, 0.00075, 0.0020]u"kg/kg"
atm.specific_humidity[:] .= q
```
...
the field `specific_humidity` of our atmosphere object is filled with values that represent a quantity in unitless dimensions of kg/kg. The atmosphere object, however, was earlier defined in g/kg, as can be inferred:
...
```{julia}
atm.specific_humidity[:] .= q .|> atm.specific_humidity_unit .|> ustrip
# This should now be in g/kg!
```

The output in the above section confirms that the correction conversion was performed before the values were copied into `atm.specific_humidity`.


The last quantity to be covered in this section is the temperature profile. We can use the same recipes that we have learned before to add a temperature profile. Temperature units, however, require a litte more care than other units. A well-written article is found within the `Unitful.jl` documentation: [link](https://juliaphysics.github.io/Unitful.jl/stable/temperature/). In short, there are ambiguities with respect to mathematical operations that involve temperatures with different units, in particular if one mixes absolute  (Kelvin, Rankine) and relative (Celcius, Fahrenheit) scales. To avoid downstream issues, the functions and objects that currently accept temperature units will force users to use absolute temperature units, such as K, mK, Ra, etc.

```{julia}
T = [253., 233., 238., 253., 278., 293.]u"K"
atm.temperature[:] = ustrip(T .|> atm.temperature_unit)
```
...
```{julia}
RE.ingest!(atm, :specific_humidity, q)
```

Note that there are two specialized implementations of the function, so both the number-type and array-type quantities can be ingested into RetrievalToolbox objects very efficiently this way. Also, the target must be valid in the sense that the RetrievalToolbox object must possess both a field, say `obj.field`, and the accompanying `obj.field_unit`.
...
```julia
atm.gravity[:] .= my_own_gravity_function()
```
...
```{julia}
atm.gravity[:] .= 9.81; # this will be in m/s^2
atm.altitude[:] .= [45.0, 20.0, 10.0, 7.0, 3.5, 0.6]; # this will be in "km"
```

# Adding Atmosphere Elements

We have added everything needed for a functioning atmosphere: we defined a pressure level grid for the retrieval (`pressure_levels`), and then another pressure level grid for our meteorological profiles (`met_pressure`). We then assigned our temperature and specific humidity profiles (`temperature`, `specific_humidity`) as well as altitude and gravity (`altitude`, `gravity`).
...
```julia
if RE.findanytype(atm.atm_elements, AbstractRayleighScattering)
    # Check if we have Rayleigh scattering as an atmospheric element..

    # Calculate OD profiles
    RE.calculate_rayleigh_optical_depth_profiles!(opt, scene)
end
```
