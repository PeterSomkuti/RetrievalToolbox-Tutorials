...
::: {.callout-note}
Currently, only the NASA JPL-type ABSCO data format is accepted, and those produced by the ReFRACtor/ABSCO toolset found [here](https://github.com/ReFRACtor/ABSCO). To learn more about ABSCO tables, visit <https://disc.gsfc.nasa.gov/information/glossary?title=OCO-2%20ABSCO>.
:::

The code cell below imports needed modules and sets some good default values for plotting occasional figures. Note that these tutorials use the [JuliaMono font](https://juliamono.netlify.app), since that font has good support for many of the Unicode glyphs that we make use of (e.g. Greek letters). Adjust these parameters if you are not able to install the JuliaMono font, or the figures do not look optimal on your computer.
...
A reminder: to obtain the file below, download it from [https://github.com/RetrievalToolbox/RetrievalToolbox-Tutorials/blob/main/data/o2_spectroscopy_demo.h5](https://github.com/RetrievalToolbox/RetrievalToolbox-Tutorials/blob/main/data/o2_spectroscopy_demo.h5), and place it in a `data` subdirectory, or adjust the code below to reflect the path on your computer.
...
At first glance, array slicing is very similar to that of the Numpy Python library, or Fortran array operations. If some array `A` is declared, then using the colon `:` allows to **slice** through one or more axes. To illustrate, we can define a matrix `A`, which we set to have four rows and 3 columns:
...
The four axes of the absorption cross sections are accessed via `absco_o2.temperatures`, `absco_o2.pressures`, `absco_o2.broadener_vmrs` and `absco_o2.wavelength`.
