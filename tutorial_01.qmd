...
![Relationship between `GasAbsorber` and the underlying spectroscopy. Note that the placeholders `T1`, `T2`, `T3` and `T4` can be any combination of real-valued numbers. See the G3RT documentation for more details.](./fig_gas_spec.svg){#fig-gas-spectroscopy-struct}
